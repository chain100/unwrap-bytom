...
![bytom-transaction1](./images/bytom-transaction1.png)
...
```json
{
  "inputs": [
    {
      "type": "spend",
      "amount": 41250000000,
      "address": "sm1qzzx9hgynf9g6zf642glc58ly9fkzgdp02nlten",
      ...
      "control_program": "0014108c5ba0934951a12755523f8a1fe42a6c24342f",
      "spent_output_id": "0028cc2c4540d4a571bd0dcfa272e48b232dca4e0fef3a72b9ad86850584a572",
      "account_id": "0DTF12DMG0A02",
  ...
  "outputs": [
    {
      "type": "control",
      "id": "b23ca503457cfb7db6f5c7e173b322e0fa1154a493b856154f24da0f47d91c2d",
      "position": 0,
      "amount": 17849562600,
      "address": "sm1qkygusy2dclhqypg9nqpzk359tl2g95nnh4mzxm",
      "asset_id": "ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff",
      "asset_alias": "BTM",
      "asset_definition": {
        "decimals": 8,
      ...
      "account_id": "0DTF12DMG0A02",
      "account_alias": "freewind",
      "control_program": "0014b111c8114dc7ee02050598022b46855fd482d273"
    },
    {
      "type": "control",
      "id": "b8c873d6a51f12ab25fdf691c96a50af2384a22e6ff7848f6e4dd80e487399c7",
      "position": 1,
      "amount": 23400000000,
      ...
      "account_id": "0DTKK0DU00A02",
      "account_alias": "bytom",
      "control_program": "0014cd451eb3b1ab9949c9a8cdb086fc78b60b444ff2"
    }
  ],
  "id": "6866c1ab2bfa2468ce44451ce6af2a83f3885cdb6a1673fec94b27f338acf9c5",
  "timestamp": 1526559212,
  "position": 1,
  "confirmations": 60,
  "tx_id": "6866c1ab2bfa2468ce44451ce6af2a83f3885cdb6a1673fec94b27f338acf9c5",
  "block_time": 1526559212,
  "block_hash": "3b61fc6e6669fab7b3e779a649742d21fba6eb89a4c818fbd5d9c825016cf094",
  "block_height": 5529,
  "block_index": 1,
  "block_transactions_count": 2,
  "status_fail": false,
  "block_id": "3b61fc6e6669fab7b3e779a649742d21fba6eb89a4c818fbd5d9c825016cf094"
}
```
...
5. 最后的JSON与图3的内容是对应的，如何解读？

今天我文章，我们主要是研究前两个问题，即跟图1相关的逻辑。
...
由于上面的代码实在太长太细节，全是一些jsx用于生成表单的代码，我们就跳过算了，有兴趣的同学可以看细节。我们需要关注的是，当我们单击了“提交交易”的按钮以后，`this.submitWithValidation`会被调用，而它对应的代码是：
...
通常我们应该会在提交处理函数中找到一些线索，用来发现数据到底会提交到后台哪个接口。但是从这里好像看不到什么信息，只有一个来自于`props`的看起来非常通用的`submitForm`，看来需要多找找线索。
...
我把不太关注的内容都省略了，需要关注的是`BaseNew.mapDispatchToProps('transaction')(dispatch)`这一行。为什么要关注`mapDispatchToProps`呢？这是因为当我们点击了表单中的提交按钮后，不论中间怎么操作，最后一定要调用`dispatch`来处理某个action。而在前面看到，点击“提交交易”后，执行的是`this.props.submitForm`，可以看出，这个`submitForm`是从外部传进来的，而`mapDispatchToPros`就是把dispatch操作映射在`props`上，让`props`中有函数可以让我们调用，所以如果我们不能从其它地方看到明显的线索的时候，应该考虑去看看这个。
...
果然在里面找到了`submitForm`的定义。在里面第一个`dispatch`处，传入了参数`actions[type].submitForm(data)`，这个`type`根据前面的代码，应该是`transaction`，而`actions`应该是从之前某处定义的各种action的集合，根据`import actions from 'actions'`，由于后面的`'actions'`不是相对路径，所以我们可以从js的源代码根目录`src`下找到`actions.js`，打开一看，果然找到了`transaction`：
...
可以看出来有一些字段比较`base_transaction`, `ttl`, `time_range`等在本例中并没有提交上来，它们应该是可选的。
...
![go-generic](./images/go-generic.png)
...
1. 第1处代码是对交易对象签名相关的内容进行严格的检查，比如参数个数、签名、甚至某些对应虚拟机的操作码，这一块挺复杂的，你一定不会想看[blockchain/txbuilder/finalize.go#L66-L113]([blockchain/txbuilder/finalize.go#L66-L113)](https://github.com/freewind/bytom-v1.0.1/blob/master/blockchain/txbuilder/finalize.go#L66-L113))
