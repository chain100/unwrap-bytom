比原是如何通过`/create-account`接口创建帐户的
=========================================
...
可以看到，它需要前端传过来`root_xpubs`、`quorum`和`alias`这三个参数，我们在之前的文章中也看到，前端也的确传了过来。这三个参数，通过`jsonHandler`的转换，到这个方法的时候，已经成了合适的GO类型，我们可以直接使用。

这个方法主要分成了三块：

1. 使用`a.wallet.AccountMgr.Create`以及用户发送的参数去创建相应的帐户
2. 调用`account.Annotated(acc)`TODO
