...
在本文的开始，我们说过希望能找到一种方法修改比原的代码，让我们在`solonet`模式下，可以正常挖矿，得到BTM用于测试。看到这个方法的时候，我觉得已经找到了，我们只需要修改一下它让它永远返回`true`即可：
...
为什么我们要使用`if`这样的方式来修改，而不是直接把函数整个内容替换成`return true`呢？这是因为如果把整个内容换了，可能会有一些import处于“未使用”状态，编译出错还要修正。而这种方法就不会，并且以后删起来也方便。

这样修改完以后，再重新编译并启动比原节点，每个块都能挖到了（一下子变成大富豪了：）
