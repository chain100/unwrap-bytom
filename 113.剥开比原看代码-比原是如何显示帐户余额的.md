...
3. action是用来向reducer传递数据的，reducer将根据action的类型和参数来做不同的转换
...
可以看到除了`reducers`，它还暴露了另的，那些我们一会儿再研究。先看`reducers`，它对应于`reducers.js`：
...
可以看到，它是把两种作用的reducer合并起来了，一个是跟元素相关的，另一个是用来记录查询状态的（是否查询过）。
...
2. 第2处根据传入的参数来确定这个router里到底有哪些routes，比如是否需要“新建”，“显示”等等
...
3. 对accountId进行排序，因为想让结果稳定并且容易看
4. 对assetId也进行排序
