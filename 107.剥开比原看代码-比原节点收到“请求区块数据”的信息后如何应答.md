...
如果我们对前一篇《比原是如何把请求区块数据的信息发出去的》有印象的话，会记得比原在发送信息时，最后会把信息写入到`MConnection.bufWriter`中；与之相应的，`MConnection`还有一个`bufReader`，它是与`net.Conn`绑定在一起的：

https://github.com/freewind/bytom/blob/527ce84a99331cf66fc77d69e48d9fce1881facf/p2p/connection.go#L114-L118
...
https://github.com/freewind/bytom/blob/527ce84a99331cf66fc77d69e48d9fce1881facf/p2p/connection.go#L152-L159
...
https://github.com/freewind/bytom/blob/527ce84a99331cf66fc77d69e48d9fce1881facf/p2p/connection.go#L403-L502
...
1. 第一块就限制接受速率，以防止恶意结点突然发送大量数据把节点撑死。跟发送一样，它的限制是`500K/s`
2. 第二块是从`c.bufReader`中读取出下一个数据包的类型。它的值目前有三个，两个跟心跳有关：`packetTypePing`和`packetTypePong`，另一个表示是正常的信息数据类型`packetTypeMsg`
...
我看重点是看第三块内容，首先是`channel.recvMsgPacket(pkt)`，即通道是怎么从packet包里读取到相应的二进制数据的呢？

https://github.com/freewind/bytom/blob/527ce84a99331cf66fc77d69e48d9fce1881facf/p2p/connection.go#L667-L682
...
https://github.com/freewind/bytom/blob/f6379ffc4c3067dae4675d19c91dff977cf11af9/p2p/peer.go#L292-L310
...
果然，在这里将会针对`BlockRequestMessage`类型的信息，进行相应的处理。处理的代码我在这里暂时省略了，因为它是属于下一个小问题的，我们先不考虑。

我们在前面说，读取`bufReader`的代码的起点是在`MConnection.Start`中，那么比原又是在什么情况下会调用到`MConnection.Start`呢？这个问题在前一篇文章《比原是如何把请求区块数据的信息发出去的》中进行了专门的讨论，有需要的话可以再过去看一下（可以先看最后“总结”那一小节）。

现在基本上就把第一个小问题弄清楚了，下面我们进入第二个小问题：
...
可以看到，逻辑还是比较简单的，即根据对方发过来的`BlockRequestMessage`中指定的`height`或者`hash`信息，在本地的区块链数据中找到相应的block，组成`NewBlockResponseMessage`发过去就行了。

其中`chain.GetBlockByHeight(...)`和`chain.GetBlockByHash(...)`如果详细说明的话，需要深刻理解区块链数据在比原节点中是如何保存的，我们在本文先不讲，等到后面专门研究。

在这里，我觉得我们只需要知道我们会查询区块数据并且构造出一个`NewBlockResponseMessage`，再通过`BlockchainChannel`这个通道发送出去就可以了。

最后一句，调用了`src.TrySend`方法，把信息向对方peer发了出去。（其中的`src`就是指的对方peer）

那么，它到底是怎么发送出去的呢？下面我们进入最后一个小问题：

这个`NewBlockResponseMessage`信息是如何发送出去的？
----------------------------------------------

我们先看看`peer.TrySend`代码：

https://github.com/freewind/bytom/blob/f6379ffc4c3067dae4675d19c91dff977cf11af9/p2p/peer.go#L242-L247
...
再接着就是`MConnection.TrySend`，由于它在前一篇文章中进行了全面的讲解，在本文就不提了，如果需要可以过去翻看一下。
